  ...
  ({
    ...
    const getStaggerDelay = useCallback((index: number) => {
      const total = splitBy === "characters"
        ? elements.reduce((acc, word) => acc + (typeof word === "string" ? 1 : (word as WordObject).characters.length + ((word as WordObject).needsSpace ? 1 : 0)), 0)
        : elements.length
              ...
              {wordObj.characters.map((char, charIndex) => (
                <span className={cn(elementLevelClassName, "whitespace-pre-wrap relative")} key={charIndex}>
                  <motion.span
                    ...
                    initial="hidden" animate={isAnimating ? "visible" : "hidden"} variants={variants}
